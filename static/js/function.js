...
async function generatePage() {
  const response = await fetch("./static/js/Books.json");
  ...
  function didiMogzauriSheavse(e) {
    ...
    switch (subsection) {
      case "დააკავშირე":
        const daakavshirePdfUrl = document.querySelector(
          ".daakavshire-pdf-link"
        );
        ...
        inputs.forEach((input) => {
          input.addEventListener("input", function () {
            ...
            let duplicateInput = [...inputs].find(
              (item) =>
                item !== currentInput && item.value === currentInput.value
            );
  ...
  burgerBtn.addEventListener("click", () => {
    ...
    const filteredBooksNames = booksNames.filter((name) => {
      if (title) {
        return name === title;
      }
      return name;
    ...
    burgBookHtml = filteredBooksNames
      .map((data) => {
        return `
  ...
  function updateActiveBurgerVisibility() {
